...
public class DeSerialize extends AbstractSerializeDeserialize {
    ...
    public EObject it(InputStream reader, String sUri) {
        ...
        try {
            ...
            String xml = inputStreamToString(reader);
            ...
            xml = xml.replace(imGood, imBad);
            xml = xml.replace(iGood, iBad);
            xml = xml.replace(patGood, patBad);
            xml = xml.replace("supportingimmunization", "supportingImmunization");
                        
            InputStream workaround = stringToInputStream(xml);
            
            resource = resourceSet.createResource(uri);
            // resource.load(reader, Collections.EMPTY_MAP);
            resource.load(workaround, Collections.EMPTY_MAP);

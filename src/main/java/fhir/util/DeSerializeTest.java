...
public class DeSerializeTest {
	...
	public void testParametersXML() {
		Parameters params = createParameters();
		Serialize save = new Serialize();
		String s = save.it(params, "/ForecastParameters.structuredefinition.xml");
	...
	public Parameters createParameters() {
		...
		immunization.setDate(FHIRUtil.convertDateTime(new Date()));
                CodeableConcept cvx = FhirFactory.eINSTANCE.createCodeableConcept();
                cvx.setText(FHIRUtil.convert("123"));
                immunization.setVaccineCode(cvx);
...
//            assertNotNull(eObject);
  //          assertTrue(eObject instanceof StructureDefinition);
		StructureDefinition def = (StructureDefinition) eObject;
    System.out.println(def.getDate());
*/
            
            DeSerializeTest dst = new DeSerializeTest();
            Parameters params = dst.createParameters();
            String serial = dst.seri.it(params, "*.xml");
